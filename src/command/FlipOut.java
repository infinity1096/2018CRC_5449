...
public class FlipOut extends Command {
	...
	protected void initialize() {
		...
		error_acc = 0;
		error[0] = 0;
		error[1] = RobotMap.INTAKE_FLIP_OUT_POSE - Robot.intake.get();
	}

	// Called repeatedly when this Command is scheduled to run
	protected void execute() {
		double P_output, D_output,I_output , dt, output,gravity_offset;
		error[0] = RobotMap.INTAKE_FLIP_OUT_POSE - Robot.intake.get();
		...
		gravity_offset = 0.17 * Math.cos(Math.PI/72.0d * Robot.intake.get() / 2);
